...
class TestLazyRegions < Minitest::Test
  ...
  # Only the parameterless-lambda registration is a region — its span runs
  # from the opening brace to the closing one; a lambda that takes
  # parameters is the program's own. With no file marks there is nothing
  # else to span.
  def test_spans_recognize_the_registration_shape
    assert_equal [[19, 49]], Ryac::LazyRegions.spans(@root, [], CODE.bytesize)
  end

  def test_contains_is_the_lambda_body
    spans = Ryac::LazyRegions.spans(@root, [], CODE.bytesize)
    statements = @root.statements.body
    class_node = statements.fetch(1).arguments.arguments.fetch(1).body.body.fetch(0)
    assert Ryac::LazyRegions.contains?(spans, class_node)
    refute Ryac::LazyRegions.contains?(spans, statements.fetch(1))
    refute Ryac::LazyRegions.contains?(spans, statements.fetch(3))
  end
