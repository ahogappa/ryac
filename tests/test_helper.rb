...
module MinifyTestHelper
  ...
  def minify_at_level(code, level, verify_output: true, rbs_files: {}, lazy_sources: [])
    stages = level.is_a?(Symbol) ? Ryac::Minifier::STAGES.fetch(level) : STAGE_RECIPES.fetch(level)
    result = Ryac::Minifier.run_stages(code, stages, rbs_files: rbs_files, lazy_sources: lazy_sources)
